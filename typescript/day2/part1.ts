...
const solution = () => {
    ...
    const categorised = games.map(game => {
        return {
            gameId: game.gameId,
            picks: game.picks.reduce((acc: { [key: string]: number }, curr: string) => {
    ...
    const filtered = categorised.filter(game => game.picks.red <= 12 && game.picks.blue <= 14 && game.picks.green <= 13);
